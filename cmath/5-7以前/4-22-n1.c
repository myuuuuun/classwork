...
int mat_solve(matrix *mat1, matrix mat2, matrix mat3){
  ...
  if(check_square(mat2) != 0 || check_base(mat3, mat2.row, 1) != 0 || check_base(*mat1, mat2.row, 1) != 0 ){
    return -1;
  }
...
// Gaussの消去法
  for(i=0; i<a.row; i++){
    ...
    for(j=i+1; j<a.row; j++){
      ...
      b.element[j][0] = b.element[j][0] - b.element[i][0] * multiplier;
    }
...
// 後退代入
  ...
  for(i=a.row-1; i>=0; i--){
    ...
    for(j=a.row-1; j>i; j--){
      ...
      b.element[i][0] = b.element[i][0] - divisor * b.element[j][0];
    }

    b.element[i][0] = b.element[i][0] / a.element[i][i];
    a.element[i][i] = 1.0;
  }
  mat_copy(mat1, b);
