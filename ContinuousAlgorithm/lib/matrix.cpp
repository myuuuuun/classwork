...
int mat_alloc(matrix *mat, int row, int col){
  ...
  mat->row = row;
  mat->col = col;
  mat->element = (double **)calloc(row, sizeof(double*));
  mat->base_element = (double *)calloc(col * row, sizeof(double));
...
// aのn列目に対してピボット選択を行い、a, bに関して行を入れ替える
int pivot(matrix *a, matrix *b, int n){
  ...
  max_row = n;
  // printf("\n%d\n", max_row);
  // n行目が最初の行。n行目以降にn行目よりもn列目の値の絶対値が大きい行がないか探す。
  for(i=n+1; i<a->row; i++){
    if(fabs(a->element[i][n]) - fabs(a->element[max_row][n]) > MAT_EPSILON){
      max_row = i;
      // printf("\n%d\n", max_row);
    }
  }
